...
def fake_project_dir():
    ...
    try:
        ...
        with open(
            os.path.join(tmpdir, "project_root", "my_module1", "requirements.txt"),
            "w",
        ):
            pass
...
def test_create_model_bundle_from_dirs_bundle_contents_correct(requests_mock, fake_project_dir):  # noqa: F811
    def check_bundle_upload_data(request):
        ...
        try:
            ...
            assert actual_zip_filenames == set(["my_module1/requirements.txt", "my_module2/foobar.txt"])
            return True
        finally:
            zf.close()
    ...
    client.create_model_bundle_from_dirs(
        ...
        requirements_path=os.path.join(fake_project_dir, "project_root/my_module1/requirements.txt"),
        env_params={
