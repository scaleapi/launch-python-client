...
class OpenApiModel(object):
    ...
    def set_attribute(self, name, value):
        ...
        if (name,) in self.validations:
            check_validations(
                self.validations, (name,), value, self._configuration
            )
...
def check_allowed_values(allowed_values, input_variable_path, input_values):
    ...
    elif isinstance(input_values, dict) and not set(
        input_values.keys()
    ).issubset(set(these_allowed_values)):
        invalid_values = ", ".join(
...
def check_validations(
    ...
    if (
        ...
        and not (
            float(input_values) / current_validations["multiple_of"]
        ).is_integer()
    ):
    ...
    if (
        ...
        err_msg = (
            r"Invalid value for `%s`, must match regular expression `%s`"
...
def get_discriminated_classes(cls):
    ...
    for discr_cls in cls.discriminator[key].values():
        if (
            hasattr(discr_cls, "discriminator")
            and discr_cls.discriminator is not None
        ):
    ...
    for valid_class in valid_classes_coercible:
        try:
            ...
            return deserialize_primitive(
                input_value, valid_class, path_to_item
            )
    ...
    if not valid_type and (
        ...
        for valid_class in valid_classes:
            if input_class_simple is none_type and is_type_nullable(
                ...
                return True
            if not (
                issubclass(valid_class, OpenApiModel)
...
def validate_and_convert_types(
    ...
    if not valid_type:
        if configuration or (
            input_class_simple == dict and dict not in valid_classes
        ):
